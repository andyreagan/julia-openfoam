...
We create both 2-dimensional and 3-dimensional meshes using OpenFOAM's native meshing utility \verb|blockMesh|.
...
From these tests, we find the optimal data assimilation parameters (inflation factors) for predicting time series with this system.
...
\begin{figure}[t]
  ...
  \caption[]{
    ...
    While we are motivated by localization around flow structures like Panel C, we simply shifted the covariance in Panel A so that our method is most general and computationally efficient.
  }
...
We emply the Dynamic Mode Decomposition, specifically the ``standard'' algorithm of Tu \cite{tu2013dynamic}, using snapshots every 10 seconds for the first 900 seconds of model time.
