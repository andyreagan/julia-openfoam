...
# this will go read the case from T and reshape it
...
# # test rewriting a variable in a case
# println("rewriting T at time 0")
# rewriteVar(case,"0","T",truth)
println("rewriting T at time 0")
...
x,y = size(points)
# loop over zones
# only the very first zone
local_obs = zeros(y,R*2+1)
local_obs_flat = zeros(length(local_obs))
...
# for i in 0:x-1
    ...
    local_obs = squeeze(observations[20,mod(linspace(i-R,i+R,R*2+1),x)+1,:],1)'
